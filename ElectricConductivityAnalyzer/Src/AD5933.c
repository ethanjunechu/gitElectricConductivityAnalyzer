...
long hz_start = 1000;	//扫描频率开始
long hz_end = 10000;	//扫描频率结束
...
void Write_Byte(char nAddr, unsigned int nValue) //nAddr中写入字节nValue
{
...
//	uint16_t i;
	...
	//navle = resistance[0];
	navle = resistance[AD5933_Conv_Num - 1];
...
//	AD5933_Sweep(hz_start, 40, 20, AD5933_OUTPUT_2V, AD5933_Gain_1, AD5933_Fre_UP);
	Rs = Get_resistance(AD5933_Conv_Num);
	re = Rs * cos(rads[max_i] - rads_Correction[range]);
